...
export default async function updateShipping({
  ...
  console.log(
    `body.order.shipping.address.postal_code from updateShipping ${body.order.shipping.address.postal_code}`
  );
  let shippingMethods = [];
  ...
  shippingMethods = shippingOptions.map(option => option.value(subtotal));
  ...
  let itemTax = Math.ceil(subtotal * 0.08);

  let tax0 = Math.ceil(ship0 * 0.08);
  let tax1 = Math.ceil(ship1 * 0.08);
  let tax2 = Math.ceil(ship2 * 0.08);
